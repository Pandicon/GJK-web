...
pub async fn callback() -> axum::response::Response<axum::body::Body> {
	match crate::USER_DB.lock().unwrap().as_ref().unwrap().get() {
		Ok(ul) => {
			let mut l = UserList{users: std::vec::Vec::with_capacity(ul.len())};
			for (m, p) in ul.iter() {
		...
		Err(e) => {
			...
			(axum::http::StatusCode::INTERNAL_SERVER_ERROR, [(axum::http::header::CONTENT_TYPE, "application/json")], "{\"message\":\"couldn't get user list\"}").into_response()
		}
	}
}
